...
WORKDIR /tmp
RUN set -x \
  && wget https://fastdl.mongodb.org/linux/mongodb-linux-x86_64-amazon2-4.4.18.tgz --quiet \
  && tar xvzf mongodb-linux-x86_64-amazon2-4.4.18.tgz > /dev/null \
  && mv mongodb-linux-x86_64-amazon2-4.4.18/bin/* /usr/local/bin/ \
  && rm -rf mongodb-linux-x86_64-amazon2-4.4.18*
